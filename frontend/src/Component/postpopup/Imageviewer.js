...
const Imageviewer = ({
  ...
  return (
    <div
      className={`${
        images && images.length && "overflow-y-auto main-menu h-[380px]"
      }`}
    >
      <Emojipicker users={users} setText={setText} text={text} changepart />
      <div className="border border-[#D5D7DA] border-solid rounded-md p-2 ">
        <input
          type="file"
          multiple
          className="hidden"
        ...
        {images && images.length ? (
          ...
          <>
            <div
              className="bg-[#f9f9f9] rounded-md relative p-5 h-[250px] flex items-center justify-center cursor-pointer hover:bg-[#EAEBED]"
              onClick={() => chooseFile.current.click()}
            >
              <div
                className="w-[40px] h-[40px] flex items-center justify-center bg-white rounded-full border border-[#f2f2f2] border-solid cursor-pointer hover:bg-[#F2F2F2] absolute top-3 right-3"
